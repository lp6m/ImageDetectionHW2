	...
	for(int ny = 0; ny < 32; ny++){
		for(int nx = 0; nx < 64; nx++){
			...
			if(ny == 0 || nx == 0 || ny == 31 || nx == 63){
				...
				maglist.push_back(mag);
				binlist.push_back(bin_index);
			}
...
vector<int> upper_sw_rst;
vector<int> bottom_sw_rst;
void cell_histogram_host(hls::stream<ap_axis<32,1,1,1> >& magstream, hls::stream<ap_axis<32,1,1,1> >& binstream){
	upper_sw_rst.clear();
	bottom_sw_rst.clear();

	int magarray[640*480];
	int binarray[640*480];
	for(int i = 0; i < 640 * 480; i++){
		magarray[i] = magstream.read().data;
		binarray[i] = binstream.read().data;
	}
	vector<int> celldata[60][80];
	for(int y = 0; y < 60; y++){
		for(int x = 0; x < 80; x++){
			vector<int> hist_data(9);
			for(int i = 0; i < 9; i++) hist_data[i] = 0;
			int begin_index = y * 640 * 8 + x * 8;
			for(int ny = 0; ny < 8; ny++){
				for(int nx = 0; nx < 8; nx++){
					int index = begin_index + ny * 640 + nx;
					hist_data[binarray[index]] += magarray[index];
				}
			}
			celldata[y][x] = hist_data;
		}
	}
	for(int y = 0; y < 59; y++){
		for(int x = 0; x < 80; x++){
			for(int i = 0; i < 9; i++){
				upper_sw_rst.push_back(celldata[y][x][i]);
				bottom_sw_rst.push_back(celldata[y+1][x][i]);
			}
		}
	}
}
