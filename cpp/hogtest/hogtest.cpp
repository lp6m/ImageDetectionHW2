...
int main(void){
  ...
  lite_hog(gray, dst, false);
  /*for(int i = 0; i < HISTOGRAMSIZE; i++){
    cout << fixed << setprecision(9) << dst[i] << " ";
    if(i%9 == 8) cout << endl;
  }*/
}
...
void lite_hog(cv::Mat img, double* dst, bool approx_mode){
  double descriptor[HISTOGRAMSIZE]={0};
  double sum_of_block[N_BLOCK_ROW * N_BLOCK_COL]={0};
  ...
  // cout << allsum << endl;
  for(int i = 0; i < HISTOGRAMSIZE; i++){
    cout << fixed << setprecision(10) << descriptor[i] << endl;
  }
