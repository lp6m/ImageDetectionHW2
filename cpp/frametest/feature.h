...
#define N_BLOCK_ROW (N_CELLS_ROW - 1) //3
#define N_BLOCK_COL (N_CELLS_COL - 1) //7
#define HISTOGRAMSIZE N_BLOCK_ROW * N_BLOCK_COL * BLOCK_SIZE_ROW * BLOCK_SIZE_COL * 9 //3*7*2*2*9
#define FEATURESIZE (HISTOGRAMSIZE + 192*2)
...
void lite_hog(cv::Mat img, double* dst, bool approx_mode){
    ...
    for(int bx = 0; bx < N_BLOCK_COL; bx++){//7
      ...
      for (int i = 0; i < 36; i++) {
        ...
        if(sum_of_block[blkIdx] == 0){
          ...
          dst[index] = (double)descriptor[index]/(double)sum_of_block[blkIdx];
        }
      }
    }
  }
...
void ravel(cv::Mat img, double* dst){
  ...
  for(int y = 0; y < img.rows; y++){
    for(int x = 0; x < img.cols; x++){
      ...
      dst[3 * (y * img.cols + x)] = double(pix[0])/256.0;
      dst[3 * (y * img.cols + x) + 1] = double(pix[1])/256.0;
      dst[3 * (y * img.cols + x) + 2] = double(pix[2])/256.0;
    }
  }
}
...
//Input: 
//img: BGR image
//dst: pointer to save extracted feature value
void getFeature(cv::Mat img, double* dst, bool approx_mode){
  ...
  ravel(spatial_hls, dst);
  ravel(spatial_rgb, dst + 192);
  lite_hog(gray, dst+192*2, approx_mode);
}