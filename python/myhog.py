        ...
        for x in range(7):
            for yy in range(16):
                for xx in range(16):
                    ny = 8 * y + yy
                    nx = 8 * x + xx
                    bin_index = binarray[ny * 64 + nx]
                    cell_index = (yy // 8) * 2 + (xx // 8);
                    block_index = y * 7 + x;
                    descrip_index = block_index * 36 + cell_index * 9 + bin_index;

                    descriptor[descrip_index] += magarray[ny * 64 + nx];
                    sum_of_block[block_index] += magarray[ny * 64 + nx];
